...
public class PawDetection {
	...
	IplImage mouseThresholding = cvCreateImage(cvGetSize(greenChannel), 8,
		1);
	double thresh = avg.val(0);
	cvThreshold(greenChannel, mouseThresholding, thresh, MAXVAL,
		THRESH_BINARY);
	if (isDebugMode) {
	...
	// for (ptr = paws.get(0).getContour(); ptr != null && ptr.address() !=
	// 0; ptr = ptr.h_prev()) {
	// cvDrawContours(srcImage, ptr, new CvScalar(0, 0, 255, 0), new
	// CvScalar(0, 0, 255, 0), -1, CV_FILLED, 8, cvPoint(0, 0));
	// }
	...
	for (int i = 0, size = paws.size(); i < size; i++) {
	    ...
	    CvPoint endPoint = new CvPoint(xBBox + boundingBox.width(), yBBox
		    + boundingBox.height());
	    cvRectangle(pata, offesetPoint, endPoint, CvScalar.BLACK, 2, CV_AA,
		    0);
	    ...
	    BigDistance pawLength = Operations
	    ...
	    Orientation orientation = Math.abs(y) > Math.abs(x) ? Orientation.VERTICAL
		    : Orientation.HORIZONTAL;
	    double angle = Operations.radianToDegressRotate(theta, orientation);
	    System.out.println(theta);
	    System.out.println(angle);

	    IplImage rotatedPaw = Operations.rotateImage(pata, angle,
		    orientation);
	    if (isDebugMode) {
		cvSaveImage(PATH + "pata" + (i + 1) + "_rotated.jpg",
			rotatedPaw);
	    }
    ...
    public static int difineRatio(IplImage mouseThresholding, IplImage channelG) {
	...
	for (int thresh = 129; pixelsForSquare == 0 && thresh < 160; thresh += 5) {
	    ...
	    cvThreshold(filtered, filteredThresh, thresh, MAXVAL, THRESH_BINARY);
		...
		IplImage rotatedPossibleSquare = Operations.rotateImage(
			squares, angle, Orientation.VERTICAL);
		cvSaveImage(PATH + "_square_rotated_" + i + ".jpg",
		...
		if (square != null) {
		    ...
		    if (relationDifference < 13 && size > (300 * 300)
			    && size < (500 * 500)) {
	...
	// Cada quadrado possui 2x2 centímetros de tamanho, então divide por
	// dois para retornar a escala por centimetro
